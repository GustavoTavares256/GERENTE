...
- **Check-in (manhã)**: `/entrada` → "Quais são suas tarefas para hoje?" → registra lista + timestamp.
  Um novo `/entrada` no mesmo dia **acumula** tarefas ao plano (não duplica).
...
- **Manhã** (`HORA_CHECKIN`, padrão 10:00): inicia a **conversa guiada de check-in**
  ...
  colaborador responde — sem precisar digitar `/entrada`).
...
- **Fim do expediente** (`HORA_CHECKOUT`, padrão 16:30): inicia a **conversa guiada de check-out**
  ...
  (sem precisar digitar `/saida`), enviando as **sugestões individuais do colaborador**
...
- ✅ **Turnos automáticos** (`src/agendar/Scheduler.ts`): dispara nos horários configurados e
  **inicia a conversa guiada de entrada/saida** automaticamente (sem digitar comando), para a
...
- Notificações/lembretes de entrada/saida para quem ainda não registrou (fora dos turnos fixos).
...
1. Com `FUNCIONARIOS_IDS` preenchido, rodar `lembrarCheckinTodos` (ou atingir `HORA_CHECKIN`) →
   o colaborador **recebe a pergunta** sobre as tarefas, sem digitar `/entrada`.
...
- Check-in/saida duplicado no dia → bloqueado com aviso.
